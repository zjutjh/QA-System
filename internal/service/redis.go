...
func SetUserLimit(c context.Context, stuId string, sid int, limit int) error {
	// 设置用户的对该问卷的访问次数
	item := "survey:" + strconv.Itoa(sid) + ":stu_id:" + stuId
	// 获取当前时间和第二天零点的时间
	now := time.Now()
	tomorrow := time.Date(
		now.Year(),
...
func InscUserLimit(c context.Context, stuId string, sid int) error {
	// 更新用户的对该问卷的访问次数
	item := "survey:" + strconv.Itoa(sid) + ":stu_id:" + stuId
