...
func (d *Dao) UpdateSurvey(
	...
	err := d.orm.WithContext(ctx).Model(&models.Survey{}).Where("id = ?", id).
		Updates(models.Survey{
			Title:      title,
			Desc:       desc,
			Img:        img,
			Deadline:   deadline,
...
func (d *Dao) GetSurveyByTitle(ctx context.Context, title string, num, size int) ([]models.Survey, *int64, error) {
	var surveys []models.Survey
	var sum int64
	err := d.orm.WithContext(ctx).Model(models.Survey{}).
...
func (d *Dao) IncreaseSurveyNum(ctx context.Context, sid int) error {
	err := d.orm.WithContext(ctx).Model(&models.Survey{}).Where("id = ?", sid).
